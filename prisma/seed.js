const { PrismaClient, Role, Department, DayOfWeek } = require('@prisma/client');
const { faker } = require('@faker-js/faker');

const prisma = new PrismaClient();

const randomEnumValue = (enumObj) => {
		...
		Array.from({ length: 10 }).map(async () => {
			const patient = await prisma.patient.create({
				data: {
					...
					gender: randomEnumValue({ Male: 'Male', Female: 'Female' }), // Gender placeholder
